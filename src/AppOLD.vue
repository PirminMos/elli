                ...
                <div class="time-col-wrapper">
                  ...
                  <div class="custom-select-wrapper" style="flex: 1;">
                    <div class="custom-select-trigger"
                        :class="{ 'rotate': isDropdownOpen('einsatzorte-' + index)
                      }"
                        @click.stop="toggleDropdown('einsatzorte-' + index)"
                    >
                      <span>{{ einsatzorte.length > 0 ? getEinsatzortNamen(termin) : 'Einsatzort' }}</span>
                      <span class="arrow-down" :class="{ 'rotate': isDropdownOpen('einsatzorte-' + index) }">▼</span>
                    </div>

                    <transition name="fade">
                      <div v-if="isDropdownOpen('einsatzorte-' + index)" class="custom-options glass">
                        <div v-for="e in einsatzorte" :key="e.id"
                             class="custom-option"
                             :class="{ selected: termin.einsatzorte.includes(e.id) }"
                             @click.stop="toggleSelection(termin.einsatzorte, e.id)">
                          {{ einsatzorte.name }}
                          <span v-if="termin.einsatzorte.some(id => id == e.id)">✓</span>
                        </div>
                      </div>
                    </transition>
                  </div>
...
const API_URL = 'http://192.168.178.52:8080/api.php'
//const API_URL = 'https://terminkompass.com/api.php'
...
export default {
  ...
  methods: {
    ...
    async saveLehrerTermin() {
      ...
      // --- B. TERMINKONFLIKTPRÜFUNG (Lehrer-Doppelbelegung) ---
      // Basierend auf deiner PHP-Struktur ist 'this.currentLehrerstundenplan' das Objekt,
      // das direkt 'termine' enthält (oder gemappt über die Lehrer-ID).

      const lehrerDaten = this.currentLehrerstundenplan[this.activeLehrer.id] || this.currentLehrerstundenplan;

      if (lehrerDaten && lehrerDaten.termine) {
        const doppelung = lehrerDaten.termine.find(t => {
        ...
        if (doppelung) {
          ...
          this.showStatus(
              `${this.erstkraefte.find(r => r.id === this.activeLehrerId)?.name} ist bereits verplant mit "${info}" (${zeit} Uhr)`,
              "error"
          );
      ...
      if (this.lehrerPlanForm.dragMode === 'move' || this.lehrerPlanForm.dragMode === 'edit') {
        this.currentLehrerstundenplan.termine = this.currentLehrerstundenplan.termine.filter(t => {
      ...
      if (this.lehrerPlanForm.aktivitaet) {
        this.currentLehrerstundenplan.termine.push(neuerTermin);
      } else {
    ...
    isRaumVerfuegbar(raum_id, tag, start, ende, termin_id = null) {
      ...
      // Falls der Raum "immer verfügbar" markiert ist, überspringen wir die Slot-Prüfung
      if (raum.immer_verfuegbar !== 1) {
        // 2. Prüfen, ob der Zeitraum in den erlaubten 'verfuegbarkeiten' liegt
        ...
        if (!hatSlot) {
          this.showStatus(raum.name + " ist nur verfügbar am " + raum.verfuegbarkeiten.map(v => {
            return `${v.tag}: ${v.start.slice(0, 5)} - ${v.ende.slice(0, 5)} Uhr`;
          }).join(', ') + ".", "error");
          return false;
        }
      }
    ...
    getFachFarbe(name) {
      const fach = this.currentLehrerstundenplan.termine.find(f => f.fach === name);
      ...
      try {
        ...
        if (activityData.termine) {
          activityData.termine.forEach(t => {
            ...
            // D) Einsatzorte für PHP aufbereiten (NEU)
            if (!t.einsatzorte || !Array.isArray(t.einsatzorte)) {
              t.einsatzorte = []; // Sicherer Default, falls nichts ausgewählt
            } else {
              // Stelle sicher, dass alle IDs saubere Integers sind (falls es IDs sind)
              // Falls es Text-Namen sind, lass das parseInt weg
              t.einsatzorte = t.einsatzorte.map(id => parseInt(id));
            }
          });
        }
    ...
    }, getEinsatzortNamen(termin) {
      if (!this.einsatzorte || this.einsatzorte.length === 0) return 'Einsatzort';
      if (!termin || !Array.isArray(termin.einsatzorte) || termin.einsatzorte.length === 0) return 'Einsatzort';

      // Wir suchen in der Liste 'this.einsatzorte' die Namen zu den im Termin ausgewählten IDs
      const namen = termin.einsatzorte.map(id => {
        const gefunden = this.einsatzorte.find(e => e.id == id);
        return gefunden ? gefunden.name : '';
      });

      const ergebnis = namen.filter(n => n !== '').join(', ');
      return ergebnis || 'Einsatzort';
    },
  }
  ,
  async created() {
